...
export default function ProfilePT() {
  const router = useRouter();
  const [me, setMe] = useState<any>(null);
  useEffect(() => { apiFetch('/me').then(setMe).catch(() => {}); }, []);
  ...
  return (
    <WithTabBar>
      ...
      <ScrollView style={s.container} contentContainerStyle={s.content}>
        ...
        {(!me?.ptBookings || me.ptBookings.length === 0) && <Text style={s.empty}>No PT sessions yet</Text>}
        {(me?.ptBookings ?? []).map((b: any) => (
          <View key={b.id} style={[s.card, { borderLeftColor: '#8b5cf6' }]}>
            <Text style={s.cardName}>{b.instructor?.firstName} {b.instructor?.lastName}</Text>
            <Text style={s.cardMeta}>
              {b.startsAt ? fmtDate(b.startsAt) + ' · ' + fmtTime(b.startsAt) : ''}
